...
module triple =
  ...
  let triplesToStatement (t : VDS.RDF.Triple seq) =
    ...
    |> Seq.map
         ...
         R ((S(Node.from s)),
         [ for t in tx ->
             (P(Node.from t.Predicate), O(Node.from t.Object)) ]))
    |> Seq.toList
...
module resource =
  ...
  let asTriples (s,px) = seq {for (p, o) in px -> (s, p, o) }
  ...
  let optionalList = function
    | x::xs -> Some (x::xs)
    | _ -> None

  let (|Predicate|_|) pr (R(_,xs)) =
    xs |> Seq.filter (fun (P p,_) -> p = p)
       |> Seq.map (fun (_,o) -> o)
       |> Seq.toList
       |> optionalList

  let (|HasType|_|) t (R(_,xs)) =
    xs |> Seq.filter (fun (P (Uri p),O (Uri o)) ->
