...
module Ontology =
  ...
  type OWLProperty =  Uri * Characteristics * ( Constraint list )

  type ObjectProperty = OWLProperty

  type DataProperty = OWLProperty
  ...
  type Uri with
    ...
    static member fromHasUri (has : HasIRI) =
      Uri.from ((has.getIRI()).toString())
  ...
  module manager =
    ...
    let private reason (Ontology o) =
      ...
      { Ontology = reasoner.getRootOntology()
        Reasoner = reasoner
        DataFactory = o.getOWLOntologyManager().getOWLDataFactory() }
