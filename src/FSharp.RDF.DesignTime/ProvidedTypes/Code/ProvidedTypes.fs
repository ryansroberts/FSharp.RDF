...
module internal Misc = 
  let TypeBuilderInstantiationType = 
    ...
    let typeName = 
      if runningOnMono then "System.Reflection.MonoGenericClass"
      else "System.Reflection.Emit.TypeBuilderInstantiation"
    
    typeof<TypeBuilder>.Assembly.GetType(typeName)
  
  let GetTypeFromHandleMethod = typeof<Type>.GetMethod("GetTypeFromHandle")
  let LanguagePrimitivesType = 
    typedefof<list<_>>
      .Assembly.GetType("Microsoft.FSharp.Core.LanguagePrimitives")
  let ParseInt32Method = LanguagePrimitivesType.GetMethod "ParseInt32"
  
  let DecimalConstructor = 
    typeof<decimal>.GetConstructor([| typeof<int>
                                      typeof<int>
                                      typeof<int>
                                      typeof<bool>
                                      typeof<byte> |])
  
  let DateTimeConstructor = 
    typeof<DateTime>.GetConstructor([| typeof<int64>
                                       typeof<DateTimeKind> |])
  
  let DateTimeOffsetConstructor = 
    typeof<DateTimeOffset>.GetConstructor([| typeof<int64>
                                             typeof<TimeSpan> |])
...
#if FX_NO_CUSTOMATTRIBUTEDATAlet 
#endif
  ...
  isAddress s = s = ExpectedStackState.Address
  
  let // convert NewTuple to the call to the constructor of the Tuple type (only for generated types)
...
#endif
    str x = 
    if x // check: not set twice
         = null then failwith ("Null in " // FSharp.Data change: use the real variable names instead of indices, to improve output of Debug.fs
                                          // Implement overloads
                                           // REVIEW: Give it a name and type?
  ...
  override this.GetCustomAttributesData()                 = customAttributesImpl.GetCustomAttributesData()
   ...
   let notRequired // State
    ...
    // problem later on, check the list of valid abbreviations
     ...
     // state
     // members API
      // take a copy in case more elements get added
      // members API
       // take a copy in case more elements get added
        // patch up "declaring type" on provided MethodInfo
         // patch up "declaring type" on provided MethodInfo
          // patch up "declaring type" on provided MethodInfo
           // patch up "declaring type" on provided MethodInfo
    ...
    (* strict *)
     ...
     // Methods
     // Fields
      // REVIEW: name = null. Is that a valid query?!
      // Events
      // Properties
       // Review: nam = null, valid query!?
       // Nested Types
    ...
    // Allow at most one constructor, and use its arguments as the fields of the type
     ...
     // - not (argTy :> targetTy)
      ...
      // Handle the case where this is a generic method instantiated at a type being compiled
       // ?? will never happen - if method is not found - KeyNotFoundException will be raised
...
#endif
    (* REVIEW: use equality? *) 
#if FX_NO_LOCAL_FILESYSTEM// TODO: this is very fake, we rely on the fact it is never needed
  ...
  let CustomAttributeNamedArgument(memb, 
                                   arg : IProvidedCustomAttributeTypedArgument) = 
  ...
  let mkEditorHideMethodsCustomAttributeData() = 
    { new IProvidedCustomAttributeData with
        member __.Constructor = 
          typeof<TypeProviderEditorHideMethodsAttribute>.GetConstructors().[0]
        member __.ConstructorArguments = upcast [||]
        member __.NamedArguments = upcast [||] }
  ...
  let mkXmlDocCustomAttributeDataLazy (lazyText : Lazy<string>) = 
    { new IProvidedCustomAttributeData with
        member __.Constructor = 
          typeof<TypeProviderXmlDocAttribute>.GetConstructors().[0]
        member __.ConstructorArguments = 
          upcast [| CustomAttributeTypedArgument
                      (typeof<string>, lazyText.Force()) |]
        member __.NamedArguments = upcast [||] }
  
  let mkXmlDocCustomAttributeData (s : string) = 
    mkXmlDocCustomAttributeDataLazy (lazy s)
  
  let mkDefinitionLocationAttributeCustomAttributeData (line : int, column : int, 
                                                        filePath : string) = 
    { new IProvidedCustomAttributeData with
        member __.Constructor = 
          typeof<TypeProviderDefinitionLocationAttribute>.GetConstructors().[0]
        member __.ConstructorArguments = upcast [||]
        member __.NamedArguments = 
          upcast [| CustomAttributeNamedArgument
                      (typeof<TypeProviderDefinitionLocationAttribute>
                         .GetProperty("FilePath"), 
                       CustomAttributeTypedArgument(typeof<string>, filePath))
                    
                    CustomAttributeNamedArgument
                      (typeof<TypeProviderDefinitionLocationAttribute>
                         .GetProperty("Line"), 
                       CustomAttributeTypedArgument(typeof<int>, line))
                    
                    CustomAttributeNamedArgument
                      (typeof<TypeProviderDefinitionLocationAttribute>
                         .GetProperty("Column"), 
                       CustomAttributeTypedArgument(typeof<int>, column)) |] }
  ...
  type CustomAttributesImpl() = 
    ...
    let customAttributesOnce = 
      lazy [| if hideObjectMethods then 
                yield mkEditorHideMethodsCustomAttributeData()
              
              match xmlDocDelayed with
              | None -> ()
              | Some _ -> 
                customAttributes.Add
                  (mkXmlDocCustomAttributeDataLazy xmlDocDelayedText)
              
              match obsoleteMessage with
              | None -> ()
              | Some s -> 
                customAttributes.Add(mkObsoleteAttributeCustomAttributeData s)
              if hasParamArray then yield mkParamArrayCustomAttributeData()
              yield! customAttributes |]
    
    member __.AddDefinitionLocation(line : int, column : int, filePath : string) = 
      customAttributes.Add
        (mkDefinitionLocationAttributeCustomAttributeData 
           (line, column, filePath))
    member __.AddObsolete(msg : string, isError) = 
      obsoleteMessage <- Some(msg, isError)
    
    member __.HasParamArray 
      with get () = hasParamArray
      and set (v) = hasParamArray <- v
    
    member __.AddXmlDocComputed(xmlDoc : unit -> string) = 
      xmlDocAlwaysRecomputed <- Some xmlDoc
    member __.AddXmlDocDelayed(xmlDoc : unit -> string) = 
      xmlDocDelayed <- Some xmlDoc
    member this.AddXmlDoc(text : string) = this.AddXmlDocDelayed(fun () -> text)
    member __.HideObjectMethods 
      with set v = hideObjectMethods <- v
    ...
    member __.GetCustomAttributesData() = 
      [| yield! customAttributesOnce.Force()
         ...
         | None -> ()
         | Some f -> customAttributes.Add(mkXmlDocCustomAttributeData (f())) |] :> IList<_>
  ...
  let transExpr isGenerated q = 
    let rec trans q = 
      ...
      | Quotations.Patterns.TupleGet(e, i) when isGenerated -> 
        let rec mkGet ty i (e : Quotations.Expr) = 
          let pi, restOpt = 
            Reflection.FSharpValue.PreComputeTuplePropertyInfo(ty, i)
      ...
      | Quotations.Patterns.PropertySet(obj, propInfo, args, v) -> 
        match obj with
        | None -> 
          trans (Quotations.Expr.Call(propInfo.GetSetMethod(), args @ [ v ]))
        | Some o -> 
          trans 
            (Quotations.Expr.Call
               (trans o, propInfo.GetSetMethod(), args @ [ v ]))
      ...
      | Quotations.Patterns.NewUnionCase(ci, es) -> 
        trans 
          (Quotations.Expr.Call
             (Reflection.FSharpValue.PreComputeUnionConstructorInfo ci, es))
      | Quotations.Patterns.NewRecord(ci, es) -> 
        trans 
          (Quotations.Expr.NewObject
             (Reflection.FSharpValue.PreComputeRecordConstructorInfo ci, es))
      | Quotations.Patterns.UnionCaseTest(e, uc) -> 
        let tagInfo = 
          Reflection.FSharpValue.PreComputeUnionTagMemberInfo uc.DeclaringType
        ...
        let tagExpr = 
          ...
          | :? MethodInfo as tagMeth -> 
            if tagMeth.IsStatic then 
              trans (Quotations.Expr.Call(tagMeth, [ e ]))
            else trans (Quotations.Expr.Call(e, tagMeth, []))
          | _ -> 
            failwith 
              "unreachable: unexpected result from PreComputeUnionTagMemberInfo"
    ...
    and inlineByref v vexpr bexpr = 
      ...
      | Quotations.Patterns.Sequential(e', vexpr') -> 
        Quotations.Expr.Sequential(e', inlineByref v vexpr' bexpr) |> trans
      | Quotations.Patterns.IfThenElse(c, b1, b2) -> 
        Quotations.Expr.IfThenElse
          (c, inlineByref v b1 bexpr, inlineByref v b2 bexpr) |> trans
      | Quotations.Patterns.Var _ -> 
        bexpr.Substitute(fun v' -> 
          if v = v' then Some vexpr
          else None)
        |> trans
      | _ -> failwith (sprintf "Unexpected byref binding: %A = %A" v vexpr)
    ...
    and transValue (v : obj, tyOfValue : Type, expectedTy : Type) = 
      ...
      and transList (o, ty : Type, nil, cons) = 
        ...
        |> List.ofSeq
        |> fun l -> 
          List.foldBack (fun o s -> 
            Quotations.Expr.NewUnionCase(cons, 
                                         [ converter (o)
                                           s ])) l 
            (Quotations.Expr.NewUnionCase(nil, []))
        |> trans
      
      and getConverterForType (ty : Type) = 
        if ty.IsArray then fun (v : obj) -> transArray (v :?> Array, ty)
        elif ty.IsGenericType 
             && ty.GetGenericTypeDefinition() = typedefof<_ list> then 
          let nil, cons = 
          ...
          fun v -> 
            transList (v :?> System.Collections.IEnumerable, ty, nil, cons)
        else fun v -> Quotations.Expr.Value(v, ty)
  ...
  let getFastFuncType (args : list<E>) resultType = 
    ...
    let fastFuncTy = 
      ...
      | 2 -> 
        typedefof<OptimizedClosures.FSharpFunc<_, _, _>>.MakeGenericType(types)
      | 3 -> 
  ...
  let traverse f = 
    let rec fallback e = 
      ...
      | P.Let(v, value, body) -> 
        ...
        if fixedValue === value && fixedBody === body then e
        else E.Let(v, fixedValue, fixedBody)
      | ES.ShapeVar _ -> e
      | ES.ShapeLambda(v, body) -> 
        let fixedBody = f fallback body
        if fixedBody === body then e
        else E.Lambda(v, fixedBody)
      | ES.ShapeCombination(shape, exprs) -> 
        let exprs1 = List.map (f fallback) exprs
        if List.forall2 (===) exprs exprs1 then e
        else ES.RebuildShapeCombination(shape, exprs1)
    fun e -> f fallback e
    ...
    and loopCore fallback orig = 
      ...
      | DP.SpecificCall RightPipe (None, _, [ operand; applicable ]) -> 
        ...
        | P.Lambda(arg, body) -> 
          ...
          E.Let(v, fixedOperand, 
                fixedBody.Substitute(fun v1 -> 
                  if v1 = arg then Some ev
                  else None))
    ...
    and tryPeelApplications orig args = 
      ...
      | P.Let(id, applicable, (P.Lambda(_) as body)) when n > 0 -> 
        ...
        if numberOfApplication = 0 then orig
          ...
          let invoke = 
            adapted.Type.GetMethod("Invoke", 
                                   [| for arg in args -> arg.Type |])
          E.Call(adapted, invoke, args)
        else (applicable, args) ||> List.fold (fun e a -> E.Application(e, a))
      | _ -> orig
    
    and countPeelableApplications expr v n = 
      match expr with
      | P.Lambda(arg, P.Let(v1, P.Application(P.Var f, P.Var arg1), rest)) when v = f 
                                                                                && arg = arg1 -> 
        countPeelableApplications rest v1 (n + 1)
      | P.Lambda(arg, P.Application(P.Var f, P.Var arg1)) when v = f 
                                                               && arg = arg1 -> 
        n
      | _ -> n
  ...
  let transQuotationToCode isGenerated qexprf (paramNames : string []) 
      (argExprs : Quotations.Expr []) = 
    let vars = 
      argExprs |> Array.mapi (fun i e -> Quotations.Var(paramNames.[i], e.Type))
    
    let expr = 
      qexprf ([ for v in vars -> Quotations.Expr.Var v ])
    
    let pairs = Array.zip argExprs vars
    let expr = 
      Array.foldBack (fun (arg, var) e -> Quotations.Expr.Let(var, arg, e)) 
        pairs expr
  ...
  let adjustTypeAttributes attributes isNested = 
    let visibilityAttributes = 
      ...
      | TypeAttributes.NestedAssembly | TypeAttributes.NestedPrivate | TypeAttributes.NestedFamORAssem | TypeAttributes.NestedFamily | TypeAttributes.NestedFamANDAssem when not 
                                                                                                                                                                               isNested -> 
...
type ProvidedStaticParameter(parameterName : string, parameterType : Type, ?parameterDefaultValue : obj) = 
  ...
  member __.AddXmlDocComputed(xmlDoc : unit -> string) = 
    customAttributesImpl.AddXmlDocComputed xmlDoc
  ...
  override __.Attributes = 
    if parameterDefaultValue.IsNone then enum 0
    else ParameterAttributes.Optional
  ...
  override __.GetCustomAttributes(_inherit) = 
    ignore (_inherit)
    notRequired "GetCustomAttributes" parameterName
...
type ProvidedParameter(name : string, parameterType : Type, ?isOut : bool, ?optionalValue : obj) = 
  ...
  member this.IsParamArray 
    with get () = customAttributesImpl.HasParamArray
    and set (v) = customAttributesImpl.HasParamArray <- v
  
  override this.Name = name
  override this.ParameterType = parameterType
  
  override this.Attributes = 
    (base.Attributes ||| (if isOut then ParameterAttributes.Out
                          else enum 0)
  ...
  member __.GetCustomAttributesDataImpl() = 
...
type ProvidedConstructor(parameters : ProvidedParameter list) = 
  ...
  let mutable declaringType : System.Type = null
  let mutable invokeCode : option<Quotations.Expr list -> Quotations.Expr> = 
    None
  let mutable isImplicitCtor = false
  let mutable ctorAttributes = 
    MethodAttributes.Public ||| MethodAttributes.RTSpecialName
  
  let nameText() = 
    sprintf "constructor for %s" (if declaringType = null then 
                                    "<not yet known type>"
                                  else declaringType.FullName)
  
  let customAttributesImpl = CustomAttributesImpl()
  
  member this.IsTypeInitializer 
    with get () = 
      ctorAttributes.HasFlag(MethodAttributes.Static) 
      && ctorAttributes.HasFlag(MethodAttributes.Private)
    and set (v) = 
      let typeInitializerAttributes = 
        MethodAttributes.Static ||| MethodAttributes.Private
      ctorAttributes <- if v then ctorAttributes ||| typeInitializerAttributes
                        else ctorAttributes &&& ~~~typeInitializerAttributes
  
  member this.AddXmlDocComputed xmlDoc = 
    customAttributesImpl.AddXmlDocComputed xmlDoc
  member this.AddXmlDocDelayed xmlDoc = 
    customAttributesImpl.AddXmlDocDelayed xmlDoc
  member this.AddXmlDoc xmlDoc = customAttributesImpl.AddXmlDoc xmlDoc
  member this.AddObsoleteAttribute(msg, ?isError) = 
    customAttributesImpl.AddObsolete(msg, defaultArg isError false)
  member this.AddDefinitionLocation(line, column, filePath) = 
    customAttributesImpl.AddDefinitionLocation(line, column, filePath)
  member this.HideObjectMethods 
    with set v = customAttributesImpl.HideObjectMethods <- v
  member __.GetCustomAttributesDataImpl() = 
    customAttributesImpl.GetCustomAttributesData()
  ...
  member this.DeclaringTypeImpl 
    with set x = 
      if declaringType <> null then 
        failwith 
          (sprintf "ProvidedConstructor: declaringType already set on '%s'" 
             (nameText()))
      declaringType <- x
  ...
  member this.InvokeCode 
    with set (q : Quotations.Expr list -> Quotations.Expr) = 
      ...
      | Some _ -> 
        failwith 
          (sprintf "ProvidedConstructor: code already given for '%s'" 
             (nameText()))
  ...
  member this.BaseConstructorCall 
    with set (d : Quotations.Expr list -> (ConstructorInfo * Quotations.Expr list)) = 
      ...
      | Some _ -> 
        failwith 
          (sprintf "ProvidedConstructor: base call already given for '%s'" 
             (nameText()))
  ...
  member this.GetInvokeCodeInternal isGenerated = 
    ...
    | Some f -> 
      let paramNames = 
        ...
        |> List.append (if not isGenerated || this.IsStatic then []
                        else [ "this" ])
        |> Array.ofList
      transQuotationToCode isGenerated f paramNames
    | None -> 
      failwith (sprintf "ProvidedConstructor: no invoker for '%s'" (nameText()))
  ...
  member this.GetBaseConstructorCallInternal isGenerated = 
    ...
    | Some f -> 
      Some(fun ctorArgs -> 
        ...
        c, List.map (transExpr isGenerated) baseCtorArgExprs)
    | None -> None
  
  member this.IsImplicitCtor 
    with get () = isImplicitCtor
    and set v = isImplicitCtor <- v
  ...
  override this.DeclaringType = 
    declaringType |> nonNull "ProvidedConstructor.DeclaringType"
  override this.IsDefined(_attributeType, _inherit) = true
  override this.Invoke(_invokeAttr, _binder, _parameters, _culture) = 
    notRequired "Invoke" (nameText())
  override this.Invoke(_obj, _invokeAttr, _binder, _parameters, _culture) = 
    notRequired "Invoke" (nameText())
  override this.ReflectedType = notRequired "ReflectedType" (nameText())
  override this.GetMethodImplementationFlags() = 
    notRequired "GetMethodImplementationFlags" (nameText())
  override this.MethodHandle = notRequired "MethodHandle" (nameText())
  override this.GetCustomAttributes(_inherit) = 
    notRequired "GetCustomAttributes" (nameText())
...
type ProvidedMethod(methodName : string, parameters : ProvidedParameter list, returnType : Type) = 
  ...
  member this.InvokeCode 
    with set (q : Quotations.Expr list -> Quotations.Expr) = 
      ...
      | Some _ -> 
        failwith 
          (sprintf "ProvidedConstructor: code already given for %s on type %s" 
             this.Name (if declaringType = null then "<not yet known type>"
                        else declaringType.FullName))
  ...
  member this.GetInvokeCodeInternal isGenerated = 
    ...
    | Some f -> 
      let paramNames = 
        parameters
        |> List.map (fun p -> p.Name)
        |> List.append (if this.IsStatic then []
                        else [ "this" ])
        |> Array.ofList
      transQuotationToCode isGenerated f paramNames
    | None -> 
      failwith 
        (sprintf "ProvidedMethod: no invoker for %s on type %s" this.Name 
           (if declaringType = null then "<not yet known type>"
            else declaringType.FullName))
  ...
  override this.DeclaringType = 
    declaringType |> nonNull "ProvidedMethod.DeclaringType"
  ...
  override this.CallingConvention = 
    ...
    let cc = 
      if not (this.IsStatic) then cc ||| CallingConventions.HasThis
      else cc
    cc
  ...
  override this.ReturnTypeCustomAttributes = 
    notRequired "ReturnTypeCustomAttributes" this.Name
  override this.GetBaseDefinition() = notRequired "GetBaseDefinition" this.Name
  override this.GetMethodImplementationFlags() = 
    notRequired "GetMethodImplementationFlags" this.Name
  override this.Invoke(_obj, _invokeAttr, _binder, _parameters, _culture) = 
    notRequired "Invoke" this.Name
  override this.ReflectedType = notRequired "ReflectedType" this.Name
  override this.GetCustomAttributes(_inherit) = 
    notRequired "GetCustomAttributes" this.Name
  override this.GetCustomAttributes(_attributeType, _inherit) = 
    notRequired "GetCustomAttributes" this.Name
...
type ProvidedProperty(propertyName : string, propertyType : Type, ?parameters : ProvidedParameter list) = 
  ...
  let mutable getterCode : option<Quotations.Expr list -> Quotations.Expr> = 
    None
  let mutable setterCode : option<Quotations.Expr list -> Quotations.Expr> = 
    None
  let hasGetter() = getterCode.IsSome
  let hasSetter() = setterCode.IsSome
  
  let markSpecialName (m : ProvidedMethod) = 
    m.AddMethodAttrs(MethodAttributes.SpecialName)
    m
  
  let getter = 
    lazy (ProvidedMethod
            ("get_" + propertyName, parameters, propertyType, 
             IsStaticMethod = isStatic, DeclaringTypeImpl = declaringType, 
             InvokeCode = getterCode.Value) |> markSpecialName)
  let setter = 
    lazy (ProvidedMethod
            ("set_" + propertyName, 
             parameters @ [ ProvidedParameter("value", propertyType) ], 
             typeof<System.Void>, IsStaticMethod = isStatic, 
             DeclaringTypeImpl = declaringType, InvokeCode = setterCode.Value) 
          |> markSpecialName)
  let customAttributesImpl = CustomAttributesImpl()
  member this.AddXmlDocComputed xmlDoc = 
    customAttributesImpl.AddXmlDocComputed xmlDoc
  ...
  member __.GetCustomAttributesDataImpl() = 
    customAttributesImpl.GetCustomAttributesData()
  member this.AddCustomAttribute attribute = 
    customAttributesImpl.AddCustomAttribute attribute
  member this.DeclaringTypeImpl 
    with set x = declaringType <- x
  ...
  member this.GetterCode 
    with set (q : Quotations.Expr list -> Quotations.Expr) = 
      if not getter.IsValueCreated then getterCode <- Some q
      else 
        failwith "ProvidedProperty: getter MethodInfo has already been created"
  
  member this.SetterCode 
    with set (q : Quotations.Expr list -> Quotations.Expr) = 
      if not (setter.IsValueCreated) then setterCode <- Some q
      else 
        failwith "ProvidedProperty: setter MethodInfo has already been created"
  
  override this.PropertyType = propertyType
  override this.SetValue(_obj, _value, _invokeAttr, _binder, _index, _culture) = 
    notRequired "SetValue" this.Name
  override this.GetAccessors _nonPublic = notRequired "nonPublic" this.Name
  
  override this.GetGetMethod _nonPublic = 
    if hasGetter() then getter.Force() :> MethodInfo
    else null
  
  override this.GetSetMethod _nonPublic = 
    if hasSetter() then setter.Force() :> MethodInfo
    else null
  
  override this.GetIndexParameters() = 
    [| for p in parameters -> upcast p |]
  
  override this.Attributes = PropertyAttributes.None
  override this.CanRead = hasGetter()
  override this.CanWrite = hasSetter()
  override this.GetValue(_obj, _invokeAttr, _binder, _index, _culture) : obj = 
    notRequired "GetValue" this.Name
  override this.Name = propertyName
  override this.DeclaringType = 
    declaringType |> nonNull "ProvidedProperty.DeclaringType"
  ...
  override this.GetCustomAttributes(_attributeType, _inherit) = 
    notRequired "GetCustomAttributes" this.Name
  override this.IsDefined(_attributeType, _inherit) = 
    notRequired "IsDefined" this.Name

type ProvidedEvent(eventName : string, eventHandlerType : Type) = 
  inherit System.Reflection.EventInfo()
  let mutable declaringType = null
  let mutable isStatic = false
  let mutable adderCode : option<Quotations.Expr list -> Quotations.Expr> = None
  let mutable removerCode : option<Quotations.Expr list -> Quotations.Expr> = 
    None
  ...
  let adder = 
    lazy (ProvidedMethod
            ("add_" + eventName, 
             [ ProvidedParameter("handler", eventHandlerType) ], 
             typeof<System.Void>, IsStaticMethod = isStatic, 
             DeclaringTypeImpl = declaringType, InvokeCode = adderCode.Value) 
          |> markSpecialName)
  let remover = 
    lazy (ProvidedMethod
            ("remove_" + eventName, 
             [ ProvidedParameter("handler", eventHandlerType) ], 
             typeof<System.Void>, IsStaticMethod = isStatic, 
             DeclaringTypeImpl = declaringType, InvokeCode = removerCode.Value) 
          |> markSpecialName)
  let customAttributesImpl = CustomAttributesImpl()
  member this.AddXmlDocComputed xmlDoc = 
    customAttributesImpl.AddXmlDocComputed xmlDoc
  ...
  member this.AdderCode 
    ...
    and set f = 
      if not adder.IsValueCreated then adderCode <- Some f
      else failwith "ProvidedEvent: Add MethodInfo has already been created"
  
  member this.RemoverCode 
    with get () = removerCode.Value
    and set f = 
      if not (remover.IsValueCreated) then removerCode <- Some f
      else failwith "ProvidedEvent: Remove MethodInfo has already been created"
  ...
  override this.DeclaringType = 
    declaringType |> nonNull "ProvidedEvent.DeclaringType"
  override this.MemberType : MemberTypes = MemberTypes.Event
  override this.GetRaiseMethod _nonPublic = 
    notRequired "GetRaiseMethod" this.Name
  override this.ReflectedType = notRequired "ReflectedType" this.Name
  override this.GetCustomAttributes(_inherit) = 
    notRequired "GetCustomAttributes" this.Name
  override this.GetCustomAttributes(_attributeType, _inherit) = 
...
type ProvidedLiteralField(fieldName : string, fieldType : Type, literalValue : obj) = 
  ...
  override this.DeclaringType = 
    declaringType |> nonNull "ProvidedLiteralField.DeclaringType"
  ...
  override this.IsDefined(_attributeType, _inherit) = 
    notRequired "IsDefined" this.Name
  override this.SetValue(_obj, _value, _invokeAttr, _binder, _culture) = 
    notRequired "SetValue" this.Name
...
type ProvidedField(fieldName : string, fieldType : Type) = 
  ...
  override this.Name = fieldName
  override this.DeclaringType = 
    declaringType |> nonNull "ProvidedField.DeclaringType"
  override this.MemberType : MemberTypes = MemberTypes.Field
  override this.ReflectedType = notRequired "ReflectedType" this.Name
  override this.GetCustomAttributes(_inherit) = 
    notRequired "GetCustomAttributes" this.Name
...
type ProvidedSymbolType(kind : SymbolKind, args : Type list) = 
  ...
  let rec isEquivalentTo (thisTy : Type) (otherTy : Type) = 
    ...
    | (:? ProvidedSymbolType as thisTy), (:? ProvidedSymbolType as thatTy) -> 
      (thisTy.Kind, thisTy.Args) = (thatTy.Kind, thatTy.Args)
    | (:? ProvidedSymbolType as thisTy), otherTy | otherTy, 
                                                   (:? ProvidedSymbolType as thisTy) -> 
      match thisTy.Kind, thisTy.Args with
      | SymbolKind.SDArray, [ ty ] | SymbolKind.Array _, [ ty ] when otherTy.IsArray -> 
        ty.Equals(otherTy.GetElementType())
      | SymbolKind.ByRef, [ ty ] when otherTy.IsByRef -> 
        ty.Equals(otherTy.GetElementType())
      | SymbolKind.Pointer, [ ty ] when otherTy.IsPointer -> 
        ty.Equals(otherTy.GetElementType())
      | SymbolKind.Generic baseTy, args -> 
        otherTy.IsGenericType 
        && isEquivalentTo baseTy (otherTy.GetGenericTypeDefinition()) 
        && Seq.forall2 isEquivalentTo args (otherTy.GetGenericArguments())
      | _ -> false
    | a, b -> a.Equals b
  ...
  static member convType (parameters : Type list) (ty : Type) = 
    if ty.IsGenericType then 
      let args = 
        Array.map (ProvidedSymbolType.convType parameters) 
          (ty.GetGenericArguments())
      ProvidedSymbolType
        (Generic(ty.GetGenericTypeDefinition()), Array.toList args) :> Type
  ...
  override this.FullName = 
    ...
    | SymbolKind.Generic gty, args -> 
      gty.FullName + "[" + (args
                            |> List.map (fun arg -> arg.ToString())
                            |> String.concat ",")
      + "]"
    | SymbolKind.FSharpTypeAbbreviation(_, nsp, path), args -> 
      String.concat "." (Array.append [| nsp |] path) + args.ToString()
  ...
  override this.IsAssignableFrom(otherTy) = 
    ...
    | Generic gtd -> 
      if otherTy.IsGenericType then 
        ...
        let yes = 
          gtd.Equals(otherGtd) && Seq.forall2 isEquivalentTo args otherArgs
        yes
      else base.IsAssignableFrom(otherTy)
  ...
  override this.GetArrayRank() = 
    (match kind with
     | SymbolKind.Array n -> n
     | SymbolKind.SDArray -> 1
     | _ -> invalidOp "non-array type")
  
  override this.IsArrayImpl() = 
    (match kind with
     | SymbolKind.Array _ | SymbolKind.SDArray -> true
     | _ -> false)
  
  override this.IsByRefImpl() = 
    (match kind with
     | SymbolKind.ByRef _ -> true
     | _ -> false)
  
  override this.IsPointerImpl() = 
    (match kind with
     | SymbolKind.Pointer _ -> true
     | _ -> false)
  
  override this.IsPrimitiveImpl() = false
  
  override this.IsGenericType = 
    (match kind with
     | SymbolKind.Generic _ -> true
     | _ -> false)
  
  override this.GetGenericArguments() = 
    (match kind with
     | SymbolKind.Generic _ -> args |> List.toArray
     | _ -> invalidOp "non-generic type")
  
  override this.GetGenericTypeDefinition() = 
    (match kind with
     | SymbolKind.Generic e -> e
     | _ -> invalidOp "non-generic type")
  ...
  override this.HasElementTypeImpl() = 
    (match kind with
     | SymbolKind.Generic _ -> false
     | _ -> true)
  
  override this.GetElementType() = 
    (match kind, args with
     | (SymbolKind.Array _ | SymbolKind.SDArray | SymbolKind.ByRef | SymbolKind.Pointer), 
       [ e ] -> e
     | _ -> invalidOp "not an array, pointer or byref type")
  ...
  override this.Equals(other : obj) = 
    match other with
    | :? ProvidedSymbolType as otherTy -> 
      (kind, args) = (otherTy.Kind, otherTy.Args)
    | _ -> false
  
  member this.Kind = kind
  member this.Args = args
  override this.GetConstructors _bindingAttr = 
    notRequired "GetConstructors" this.Name
  
  override this.GetMethodImpl(_name, _bindingAttr, _binderBinder, 
                              _callConvention, _types, _modifiers) = 
    match kind with
    | Generic gtd -> 
  ...
  override this.GetInterface(_name, _ignoreCase) = 
    notRequired "GetInterface" this.Name
  ...
  override this.GetProperties _bindingAttr = 
    notRequired "GetProperties" this.Name
  override this.GetPropertyImpl(_name, _bindingAttr, _binder, _returnType, 
                                _types, _modifiers) = 
    notRequired "GetPropertyImpl" this.Name
  override this.GetNestedTypes _bindingAttr = 
    notRequired "GetNestedTypes" this.Name
  override this.GetNestedType(_name, _bindingAttr) = 
    notRequired "GetNestedType" this.Name
  override this.GetAttributeFlagsImpl() = 
    notRequired "GetAttributeFlagsImpl" this.Name
  
  override this.UnderlyingSystemType = 
    match kind with
    | SymbolKind.SDArray | SymbolKind.Array _ | SymbolKind.Pointer | SymbolKind.FSharpTypeAbbreviation _ | SymbolKind.ByRef -> 
      notRequired "UnderlyingSystemType" this.Name
    | SymbolKind.Generic gty -> gty.UnderlyingSystemType
  
  override this.MemberType = notRequired "MemberType" this.Name
  override this.GetMember(_name, _mt, _bindingAttr) = 
    notRequired "GetMember" this.Name
  override this.GUID = notRequired "GUID" this.Name
  override this.InvokeMember(_name, _invokeAttr, _binder, _target, _args, 
                             _modifiers, _culture, _namedParameters) = 
    notRequired "InvokeMember" this.Name
  override this.AssemblyQualifiedName = 
    notRequired "AssemblyQualifiedName" this.Name
  override this.GetConstructorImpl(_bindingAttr, _binder, _callConvention, 
                                   _types, _modifiers) = 
    notRequired "GetConstructorImpl" this.Name
  override this.GetCustomAttributes(_inherit) = [||]
  override this.GetCustomAttributes(_attributeType, _inherit) = [||]
  override this.IsDefined(_attributeType, _inherit) = false
  override this.MakeArrayType() = 
    ProvidedSymbolType(SymbolKind.SDArray, [ this ]) :> Type
  override this.MakeArrayType arg = 
    ProvidedSymbolType(SymbolKind.Array arg, [ this ]) :> Type
...
type ProvidedSymbolMethod(genericMethodDefinition : MethodInfo, parameters : Type list) = 
  ...
  let convParam (p : ParameterInfo) = 
    { new System.Reflection.ParameterInfo() with
        member this.Name = p.Name
        member this.ParameterType = 
          ProvidedSymbolType.convType parameters p.ParameterType
        member this.Attributes = p.Attributes
        member this.RawDefaultValue = p.RawDefaultValue }
  
  override this.IsGenericMethod = 
    (if this.DeclaringType.IsGenericType then 
       this.DeclaringType.GetGenericArguments().Length
     else 0)
    < parameters.Length
  
  override this.GetGenericArguments() = 
    Seq.skip (if this.DeclaringType.IsGenericType then 
                this.DeclaringType.GetGenericArguments().Length
              else 0) parameters
    |> Seq.toArray
  ...
  override this.DeclaringType = 
    ProvidedSymbolType.convType parameters genericMethodDefinition.DeclaringType
  override this.ToString() = "Method " + this.Name
  ...
  override this.IsDefined(_attributeType, _inherit) : bool = 
    notRequired "IsDefined" this.Name
  override this.ReturnType = 
    ProvidedSymbolType.convType parameters genericMethodDefinition.ReturnType
  override this.GetParameters() = 
    genericMethodDefinition.GetParameters() |> Array.map convParam
  override this.ReturnParameter = 
    genericMethodDefinition.ReturnParameter |> convParam
  override this.ReturnTypeCustomAttributes = 
    notRequired "ReturnTypeCustomAttributes" this.Name
  override this.GetBaseDefinition() = notRequired "GetBaseDefinition" this.Name
  override this.GetMethodImplementationFlags() = 
...
type ProvidedTypeBuilder() = 
  static member MakeGenericType(genericTypeDefinition, genericArguments) = 
    ProvidedSymbolType(Generic genericTypeDefinition, genericArguments) :> Type
  static member MakeGenericMethod(genericMethodDefinition, genericArguments) = 
    ProvidedSymbolMethod(genericMethodDefinition, genericArguments) :> MethodInfo

[<Class>]
type ProvidedMeasureBuilder() = 
  let unitNamesTypeAbbreviations = 
    [ "meter"; "hertz"; "newton"; "pascal"; "joule"; "watt"; "coulomb"; "volt"; 
      "farad"; "ohm"; "siemens"; "weber"; "tesla"; "henry"; "lumen"; "lux"; 
      "becquerel"; "gray"; "sievert"; "katal" ] |> Set.ofList
  let unitSymbolsTypeAbbreviations = 
    [ "m"; "kg"; "s"; "A"; "K"; "mol"; "cd"; "Hz"; "N"; "Pa"; "J"; "W"; "C"; "V"; 
      "F"; "S"; "Wb"; "T"; "lm"; "lx"; "Bq"; "Gy"; "Sv"; "kat"; "H" ] 
    |> Set.ofList
  ...
  member b.Product(m1, m2) = 
    typedefof<Core.CompilerServices.MeasureProduct<_, _>>
      .MakeGenericType [| m1; m2 |]
  member b.Inverse m = 
    typedefof<Core.CompilerServices.MeasureInverse<_>>.MakeGenericType [| m |]
    ...
    | Some(ns, unitName) -> 
      ProvidedSymbolType
        (SymbolKind.FSharpTypeAbbreviation
           (typeof<Core.CompilerServices.MeasureOne>.Assembly, ns, 
            [| unitName |]), []) :> Type
    | None -> 
      typedefof<list<int>>
        .Assembly.GetType("Microsoft.FSharp.Data.UnitSystems.SI.UnitNames." 
                          + mLowerCase)
  
  member b.AnnotateType(basicType, annotation) = 
    ProvidedSymbolType(Generic basicType, annotation) :> Type
...
type ProvidedTypeDefinition(container : TypeContainer, className : string, baseType : Type option) as this = 
  ...
  let getInterfaces() = 
    if interfaceImplsDelayed.Count > 0 then 
      let elems = interfaceImplsDelayed |> Seq.toArray
  ...
  let mutable theAssembly = 
    lazy match container with
         | TypeContainer.Namespace(theAssembly, rootNamespace) -> 
           if theAssembly = null then failwith "Null assemblies not allowed"
           if rootNamespace <> null && rootNamespace.Length = 0 then 
             failwith "Use 'null' for global namespace"
           theAssembly
         | TypeContainer.Type superTy -> superTy.Assembly
         | TypeContainer.TypeToBeDecided -> 
           failwith 
             (sprintf "type '%s' was not added as a member to a declaring type" 
                this.Name)
  ...
  let fullName = 
    lazy match container with
         | TypeContainer.Type declaringType -> 
           declaringType.FullName + "+" + className
  ...
  let patchUpAddedMemberInfo (this : Type) (m : MemberInfo) = 
    ...
    | :? ProvidedEvent as e -> e.DeclaringTypeImpl <- this
  ...
  member this.AddCustomAttribute attribute = 
    customAttributesImpl.AddCustomAttribute attribute
  member this.ResetEnclosingType(ty) = container <- TypeContainer.Type ty
  new(assembly : Assembly, namespaceName, className, baseType) = 
    new ProvidedTypeDefinition(TypeContainer.Namespace(assembly, namespaceName), 
                               className, baseType)
  new(className, baseType) = 
    new ProvidedTypeDefinition(TypeContainer.TypeToBeDecided, className, 
                               baseType)
  override this.UnderlyingSystemType = typeof<Type>
  member this.SetEnumUnderlyingType(ty) = enumUnderlyingType <- ty
  
  override this.GetEnumUnderlyingType() = 
    if this.IsEnum then enumUnderlyingType
  ...
  member this.AddMembersDelayed(makeMS : unit -> list<#MemberInfo>) = 
    membersQueue.Add(fun () -> 
      makeMS() |> List.map (fun x -> 
                    patchUpAddedMemberInfo this x
                    x :> MemberInfo))
  
  member this.AddMembers(ms : list<#MemberInfo>) = 
    ms |> List.iter (patchUpAddedMemberInfo this)
    membersQueue.Add(fun () -> ms |> List.map (fun x -> x :> MemberInfo))
  
  member this.AddMember(m : MemberInfo) = this.AddMembers [ m ]
  member this.AddMemberDelayed(m : unit -> #MemberInfo) = 
    this.AddMembersDelayed(fun () -> [ m() ])
  ...
  member this.AddAssemblyTypesAsNestedTypesDelayed(assemblyf : unit -> System.Reflection.Assembly) = 
    let bucketByPath nodef tipf (items : (string list * 'Value) list) = 
      ...
      let nodes = 
        ...
        for (keylist, v) in items do
          ...
          | [] -> ()
          | key :: rest -> 
            buckets.[key] <- (rest, v) :: (if buckets.ContainsKey key then 
                                             buckets.[key]
                                           else [])
    ...
    this.AddMembersDelayed(fun _ -> 
      let topTypes = 
        [ for ty in assemblyf().GetTypes() do
            if not ty.IsNested then 
              let namespaceParts = 
                match ty.Namespace with
                | null -> []
                | s -> s.Split '.' |> Array.toList
              yield namespaceParts, ty ]
      
      let rec loop types = 
        types |> bucketByPath (fun namespaceComponent typesUnderNamespaceComponent -> 
                   let t = 
                     ProvidedTypeDefinition
                       (namespaceComponent, baseType = Some typeof<obj>)
                   t.AddMembers(loop typesUnderNamespaceComponent)
                   (t :> Type)) (fun ty -> ty)
      
      loop topTypes)
  
  member this.DefineStaticParameters(staticParameters : list<ProvidedStaticParameter>, 
                                     apply : string -> obj [] -> ProvidedTypeDefinition) = 
    staticParams <- staticParameters
    staticParamsApply <- Some apply
  
  member this.GetStaticParameters() = 
    [| for p in staticParams -> p :> ParameterInfo |]
      ...
      if staticParams.Length <> args.Length then 
        failwith 
          (sprintf 
             ...
             staticParams.Length args.Length (fullName.Force()))
      match staticParamsApply with
      | None -> 
        failwith "ProvidedTypeDefinition: DefineStaticParameters was not called"
      | Some f -> f name args
    else 
      failwith 
        (sprintf 
  ...
  member this.DeclaringTypeImpl 
    with set x = 
      ...
      | _ -> 
        failwith 
          (sprintf "container type for '%s' was already set to '%s'" 
  ...
  override this.GetMethodImpl(name, bindingAttr, _binderBinder, _callConvention, 
                              _types, _modifiers) : MethodInfo = 
    let membersWithName = 
      [ for m in this.GetMembers(bindingAttr) do
  ...
  override this.GetField(name, bindingAttr) = 
    let fields = 
      [| for m in this.GetMembers bindingAttr do
           if m.MemberType.HasFlag(MemberTypes.Field) 
              && (name = null || m.Name = name) then 
             yield m |]
    if fields.Length > 0 then fields.[0] :?> FieldInfo
    else null
  
  override this.GetFields bindingAttr = 
    [| for m in this.GetMembers bindingAttr do
         if m.MemberType.HasFlag(MemberTypes.Field) then yield m :?> FieldInfo |]
  
  override this.GetInterface(_name, _ignoreCase) = 
    notRequired "GetInterface" this.Name
  override this.GetInterfaces() = [| yield! getInterfaces() |]
  member this.GetInterfaceImplementations() = [| yield! getInterfaces() |]
  member this.AddInterfaceImplementation ityp = interfaceImpls.Add ityp
  member this.AddInterfaceImplementationsDelayed itypf = 
    interfaceImplsDelayed.Add itypf
  member this.GetMethodOverrides() = [| yield! methodOverrides |]
  member this.DefineMethodOverride(bodyMethInfo, declMethInfo) = 
    methodOverrides.Add(bodyMethInfo, declMethInfo)
  ...
  override this.GetEvent(name, bindingAttr) = 
    let events = 
      ...
      |> Array.filter 
           (fun m -> 
           m.MemberType.HasFlag(MemberTypes.Event) 
           && (name = null || m.Name = name))
    if events.Length > 0 then events.[0] :?> EventInfo
    else null
  
  override this.GetEvents bindingAttr = 
    [| for m in this.GetMembers bindingAttr do
         if m.MemberType.HasFlag(MemberTypes.Event) then yield downcast m |]
  
  override this.GetProperties bindingAttr = 
    [| for m in this.GetMembers bindingAttr do
         if m.MemberType.HasFlag(MemberTypes.Property) then yield downcast m |]
  
  override this.GetPropertyImpl(name, bindingAttr, binder, returnType, types, 
                                modifiers) = 
    if returnType <> null then 
      failwith "Need to handle specified return type in GetPropertyImpl"
    if types <> null then 
      failwith "Need to handle specified parameter types in GetPropertyImpl"
    if modifiers <> null then 
      failwith "Need to handle specified modifiers in GetPropertyImpl"
  ...
  override this.MakeArrayType arg = 
    ProvidedSymbolType(SymbolKind.Array arg, [ this ]) :> Type
  override this.MakePointerType() = 
    ProvidedSymbolType(SymbolKind.Pointer, [ this ]) :> Type
  override this.MakeByRefType() = 
    ProvidedSymbolType(SymbolKind.ByRef, [ this ]) :> Type
  ...
  static member EraseType(t : Type) = 
    ...
    | t when t.IsGenericType && not t.IsGenericTypeDefinition -> 
      ...
      let genericArguments = 
        t.GetGenericArguments() |> Array.map ProvidedTypeDefinition.EraseType
      genericTypeDefinition.MakeGenericType(genericArguments)
    | t -> t
  ...
  override this.GetMembers bindingAttr = 
    let mems = 
      ...
      |> Array.filter 
           ...
           let isStatic, isPublic = 
             ...
             | :? PropertyInfo as p -> 
               let m = 
                 if p.CanRead then p.GetGetMethod()
                 else p.GetSetMethod()
               m.IsStatic, m.IsPublic
             | :? EventInfo as e -> 
               let m = e.GetAddMethod()
               m.IsStatic, m.IsPublic
             | :? Type as ty -> true, ty.IsNestedPublic
             | _ -> failwith (sprintf "Member %O is of unexpected type" mem)
           bindingAttr.HasFlag(if isStatic then BindingFlags.Static
    ...
    if bindingAttr.HasFlag(BindingFlags.DeclaredOnly) || this.BaseType = null then 
      mems
    else 
      let baseMems = 
        (ProvidedTypeDefinition.EraseType this.BaseType).GetMembers bindingAttr
  ...
  override this.GetNestedTypes bindingAttr = 
    ...
    |> Array.filter 
         ...
         || m.MemberType.HasFlag(MemberTypes.TypeInfo))
  ...
  override this.GetNestedType(name, bindingAttr) = 
    let nt = 
      this.GetMember
        (name, MemberTypes.NestedType ||| MemberTypes.TypeInfo, bindingAttr)
    match nt.Length with
    | 0 -> null
    | 1 -> downcast nt.[0]
    | _ -> 
      failwith 
        (sprintf "There is more than one nested type called '%s' in type '%s'" 
           name this.FullName)
  
  override this.GetAttributeFlagsImpl() = 
    adjustTypeAttributes attributes this.IsNested
  ...
  override this.MemberType = 
    if this.IsNested then MemberTypes.NestedType
    else MemberTypes.TypeInfo
  ...
  override this.InvokeMember(_name, _invokeAttr, _binder, _target, _args, 
                             _modifiers, _culture, _namedParameters) = 
    notRequired "Module" this.Name
  override this.AssemblyQualifiedName = notRequired "Module" this.Name
  
  member this.IsErased 
    with get () = 
      (attributes &&& enum (int32 TypeProviderTypeAttributes.IsErased)) 
      <> enum 0
    and set v = 
      if v then 
  ...
  member this.SuppressRelocation 
    with get () = 
      (attributes &&& enum (int32 TypeProviderTypeAttributes.SuppressRelocate)) 
      <> enum 0
    and set v = 
      if v then 
        attributes <- attributes 
                      ||| enum 
                            (int32 TypeProviderTypeAttributes.SuppressRelocate)
      else 
        attributes <- attributes 
                      &&& ~~~(enum 
                                (int32 
...
type AssemblyGenerator(assemblyFileName) = 
  ...
  let assembly = 
    System.AppDomain.CurrentDomain.DefineDynamicAssembly
      (name = assemblyName, access = AssemblyBuilderAccess.Run)
  let assemblyMainModule = assembly.DefineDynamicModule("MainModule")
  let typeMap = 
    Dictionary<ProvidedTypeDefinition, TypeBuilder>(HashIdentity.Reference)
  ...
  member __.Generate(providedTypeDefinitions : (ProvidedTypeDefinition * string list option) list) = 
    ...
    (let rec typeMembers (tb : TypeBuilder) (td : ProvidedTypeDefinition) = 
     ...
     and nestedType (tb : TypeBuilder) (ntd : Type) = 
       ...
       | :? ProvidedTypeDefinition as pntd -> 
         if pntd.IsErased then 
           invalidOp 
             ("The nested provided type " + pntd.Name 
              + "is marked as erased and cannot be converted to a generated type. Set 'IsErased' to false on the ProvidedTypeDefinition")
     ...
     for (pt, enclosingGeneratedTypeNames) in providedTypeDefinitions do
       ...
       | None -> 
         let attributes = 
           pt.Attributes 
           &&& ~~~(enum (int32 TypeProviderTypeAttributes.SuppressRelocate)) 
           &&& ~~~(enum (int32 TypeProviderTypeAttributes.IsErased))
         let attributes = adjustTypeAttributes attributes false
         let tb = 
           assemblyMainModule.DefineType(name = pt.FullName, attr = attributes)
         pt.SetAssembly null
         typeMap.[pt] <- tb
         typeMembers tb pt
       | Some ns -> 
         let otb, _ = 
           ((None, ""), ns) ||> List.fold (fun (otb : TypeBuilder option, fullName) n -> 
                                  ...
                                  let priorType = 
                                    if typeMapExtra.ContainsKey(fullName) then 
                                      Some typeMapExtra.[fullName]
                                    else None
                                  ...
                                  let tb = 
                                    ...
                                    | None -> 
                                      ...
                                      let attributes = 
                                        adjustTypeAttributes attributes 
                                          otb.IsSome
                                      
                                      let tb = 
                                        match otb with
                                        | None -> 
                                          assemblyMainModule.DefineType
                                            (name = n, attr = attributes)
                                        | Some(otb : TypeBuilder) -> 
                                          otb.DefineNestedType
                                            (name = n, attr = attributes)
    ...
    let rec convType (ty : Type) = 
      ...
      | :? ProvidedTypeDefinition as ptd -> 
        if typeMap.ContainsKey ptd then typeMap.[ptd] :> Type
        else ty
      | _ -> 
        if ty.IsGenericType then 
          ty.GetGenericTypeDefinition()
            .MakeGenericType(Array.map convType (ty.GetGenericArguments()))
        elif ty.HasElementType then 
    ...
    let ctorMap = 
      Dictionary<ProvidedConstructor, ConstructorBuilder>
        (HashIdentity.Reference)
    let methMap = 
      Dictionary<ProvidedMethod, MethodBuilder>(HashIdentity.Reference)
    ...
    iterateTypes (fun tb ptd -> 
      ...
      | Some ptd -> 
        ...
        | null -> ()
        | bt -> tb.SetParent(convType bt))
    let defineCustomAttrs f (cattrs : IList<CustomAttributeData>) = 
      for attr in cattrs do
        let constructorArgs = 
          [ for x in attr.ConstructorArguments -> x.Value ]
        
        let namedProps, namedPropVals = 
          [ for x in attr.NamedArguments do
              match x.MemberInfo with
              | :? PropertyInfo as pi -> yield (pi, x.TypedValue.Value)
              | _ -> () ]
          |> List.unzip
        
        let namedFields, namedFieldVals = 
          [ for x in attr.NamedArguments do
              match x.MemberInfo with
              | :? FieldInfo as pi -> yield (pi, x.TypedValue.Value)
              | _ -> () ]
          |> List.unzip
        
        let cab = 
          CustomAttributeBuilder
            (attr.Constructor, Array.ofList constructorArgs, 
             Array.ofList namedProps, Array.ofList namedPropVals, 
             Array.ofList namedFields, Array.ofList namedFieldVals)
        f cab
    iterateTypes (fun tb ptd -> 
      match ptd with
      | None -> ()
      | Some ptd -> 
        for cinfo in ptd.GetConstructors(ALL) do
          match cinfo with
          | :? ProvidedConstructor as pcinfo when not 
                                                    (ctorMap.ContainsKey pcinfo) -> 
            let cb = 
              if pcinfo.IsTypeInitializer then 
                if (cinfo.GetParameters()).Length <> 0 then 
                  failwith "Type initializer should not have parameters"
                tb.DefineTypeInitializer()
              else 
                let cb = 
                  tb.DefineConstructor
                    (cinfo.Attributes, CallingConventions.Standard, 
                     [| for p in cinfo.GetParameters() -> 
                          convType p.ParameterType |])
                for (i, p) in cinfo.GetParameters() 
                              |> Seq.mapi (fun i x -> (i, x)) do
                  cb.DefineParameter(i + 1, ParameterAttributes.None, p.Name) 
                  |> ignore
                cb
            ctorMap.[pcinfo] <- cb
          | _ -> ()
        if ptd.IsEnum then 
          tb.DefineField
            ("value__", ptd.GetEnumUnderlyingType(), 
             FieldAttributes.Public ||| FieldAttributes.SpecialName 
             ||| FieldAttributes.RTSpecialName) |> ignore
        for finfo in ptd.GetFields(ALL) do
          let fieldInfo = 
            match finfo with
            | :? ProvidedField as pinfo -> 
              Some
                (pinfo.Name, convType finfo.FieldType, finfo.Attributes, 
                 pinfo.GetCustomAttributesDataImpl(), None)
            | :? ProvidedLiteralField as pinfo -> 
              Some
                (pinfo.Name, convType finfo.FieldType, finfo.Attributes, 
                 pinfo.GetCustomAttributesDataImpl(), 
                 Some(pinfo.GetRawConstantValue()))
            | _ -> None
          match fieldInfo with
          | Some(name, ty, attr, cattr, constantVal) when not 
                                                            (fieldMap.ContainsKey 
                                                               finfo) -> 
            let fb = tb.DefineField(name, ty, attr)
            if constantVal.IsSome then fb.SetConstant constantVal.Value
        ...
        for minfo in ptd.GetMethods(ALL) do
          ...
          | :? ProvidedMethod as pminfo when not (methMap.ContainsKey pminfo) -> 
            let mb = 
              tb.DefineMethod
                (minfo.Name, minfo.Attributes, convType minfo.ReturnType, 
                 [| for p in minfo.GetParameters() -> convType p.ParameterType |])
            for (i, p) in minfo.GetParameters() 
                          |> Seq.mapi (fun i x -> (i, x :?> ProvidedParameter)) do
              ...
              if p.HasDefaultParameterValue then 
                do let ctor = 
                     typeof<System.Runtime.InteropServices.DefaultParameterValueAttribute>
                       .GetConstructor([| typeof<obj> |])
                   let builder = 
                     new CustomAttributeBuilder(ctor, [| p.RawDefaultValue |])
    ...
    iterateTypes (fun tb ptd -> 
      ...
      | Some ptd -> 
        ...
        let implictCtorArgs = 
          ...
          | [] -> []
          | [ pcinfo ] -> 
            [ for p in pcinfo.GetParameters() -> p ]
          | _ -> failwith "at most one implicit constructor allowed"
        
        let implicitCtorArgsAsFields = 
          [ for ctorArg in implictCtorArgs -> 
              tb.DefineField
                (ctorArg.Name, convType ctorArg.ParameterType, 
                 FieldAttributes.Private) ]
        
        let rec emitLambda (callSiteIlg : ILGenerator, v : Quotations.Var, 
                            body : Quotations.Expr, 
                            freeVars : seq<Quotations.Var>, 
                            locals : Dictionary<_, LocalBuilder>, parameters) = 
          let lambda = 
            assemblyMainModule.DefineType
              (uniqueLambdaTypeName(), TypeAttributes.Class)
          let baseType = 
            typedefof<FSharpFunc<_, _>>.MakeGenericType(v.Type, body.Type)
          ...
          let paramTypes = 
            [| for p in decl.GetParameters() -> p.ParameterType |]
          
          let invoke = 
            lambda.DefineMethod
              ("Invoke", 
               MethodAttributes.Virtual ||| MethodAttributes.Final 
               ||| MethodAttributes.Public, decl.ReturnType, paramTypes)
          ...
          for (v, f) in fields do
            ...
            copyOfLocals.[v] <- l
          let expectedState = 
            if (invoke.ReturnType = typeof<System.Void>) then 
              ExpectedStackState.Empty
            else ExpectedStackState.Value
          emitExpr (ilg, copyOfLocals, 
                    [| Quotations.Var("this", lambda)
                       v |]) expectedState body
          ...
          for (v, f) in fields do
            ...
            | true, loc -> callSiteIlg.Emit(OpCodes.Ldloc, loc)
            | false, _ -> 
              let index = parameters |> Array.findIndex ((=) v)
              callSiteIlg.Emit(OpCodes.Ldarg, index)
            callSiteIlg.Emit(OpCodes.Stfld, f)
        
        and emitExpr (ilg : ILGenerator, 
                      locals : Dictionary<Quotations.Var, LocalBuilder>, 
                      parameterVars) expectedState expr = 
          let pop() = ilg.Emit(OpCodes.Pop)
          
          let popIfEmptyExpected s = 
            if isEmpty s then pop()
          
          let emitConvIfNecessary t1 = 
            if t1 = typeof<int16> then ilg.Emit(OpCodes.Conv_I2)
            elif t1 = typeof<uint16> then ilg.Emit(OpCodes.Conv_U2)
            elif t1 = typeof<sbyte> then ilg.Emit(OpCodes.Conv_I1)
            else 
              if t1 = typeof<byte> then ilg.Emit(OpCodes.Conv_U1)
          
          let rec emit (expectedState : ExpectedStackState) 
            ...
            | Quotations.Patterns.Var v -> 
              if isEmpty expectedState then ()
                ...
                let methIdx = 
                  parameterVars |> Array.tryFindIndex (fun p -> p = v)
                match methIdx with
                | Some idx -> 
                  ilg.Emit((if isAddress expectedState then OpCodes.Ldarga
                            else OpCodes.Ldarg), idx)
                | None -> 
                  let implicitCtorArgFieldOpt = 
                    implicitCtorArgsAsFields 
                    |> List.tryFind (fun f -> f.Name = v.Name)
                  ...
                  | None -> 
                    match locals.TryGetValue v with
                    | true, localBuilder -> 
                      ilg.Emit((if isAddress expectedState then OpCodes.Ldloca
                                else OpCodes.Ldloc), localBuilder.LocalIndex)
                    | false, _ -> failwith "unknown parameter/field"
            | Quotations.Patterns.Coerce(arg, ty) -> 
              ...
              if argTy.IsValueType && not targetTy.IsValueType then 
                ...
                if targetTy <> typeof<obj> 
                   && (Misc.TypeBuilderInstantiationType.Equals
                         (targetTy.GetType()) 
                       || not (targetTy.IsAssignableFrom(argTy))) then 
            ...
            | Quotations.DerivedPatterns.SpecificCall <@ (-) @> 
              ...
              if t1 = typeof<decimal> then 
                ilg.Emit
                  (OpCodes.Call, typeof<decimal>.GetMethod "op_Subtraction")
              else 
            ...
            | Quotations.DerivedPatterns.SpecificCall <@ (/) @> 
              ...
              if t1 = typeof<decimal> then 
                ...
                | TypeCode.UInt32 | TypeCode.UInt64 | TypeCode.UInt16 | TypeCode.Byte | _ when t1 = typeof<unativeint> -> 
                  ilg.Emit(OpCodes.Div_Un)
                | _ -> ilg.Emit(OpCodes.Div)
                emitConvIfNecessary t1
              popIfEmptyExpected expectedState
            | Quotations.DerivedPatterns.SpecificCall <@ int @> 
              (None, [ sourceTy ], [ v ]) -> 
              emit ExpectedStackState.Value v
              match Type.GetTypeCode(sourceTy) with
              | TypeCode.String -> ilg.Emit(OpCodes.Call, Misc.ParseInt32Method)
              | TypeCode.Single | TypeCode.Double | TypeCode.Int64 | TypeCode.UInt64 | TypeCode.UInt16 | TypeCode.Char | TypeCode.Byte | _ when sourceTy = typeof<nativeint> 
                                                                                                                                                || sourceTy = typeof<unativeint> -> 
                ilg.Emit(OpCodes.Conv_I4)
              | TypeCode.Int32 | TypeCode.UInt32 | TypeCode.Int16 | TypeCode.SByte -> 
                ()
              | _ -> failwith "TODO: search for op_Explicit on sourceTy"
            | Quotations.DerivedPatterns.SpecificCall <@ LanguagePrimitives.IntrinsicFunctions.GetArray @> 
              (None, [ ty ], [ arr; index ]) -> 
              ...
              else ilg.Emit(OpCodes.Ldelem, convType ty)
              popIfEmptyExpected expectedState
            | Quotations.DerivedPatterns.SpecificCall <@ LanguagePrimitives.IntrinsicFunctions.GetArray2D @> 
              (None, _ty, arr :: indices) | Quotations.DerivedPatterns.SpecificCall <@ LanguagePrimitives.IntrinsicFunctions.GetArray3D @> 
                                            (None, _ty, arr :: indices) | Quotations.DerivedPatterns.SpecificCall <@ LanguagePrimitives.IntrinsicFunctions.GetArray4D @> 
                                                                          (None, 
                                                                           _ty, 
                                                                           arr :: indices) -> 
              let meth = 
                let name = 
                  if isAddress expectedState then "Address"
                  else "Get"
                arr.Type.GetMethod(name)
              emit ExpectedStackState.Value arr
              for index in indices do
                emit ExpectedStackState.Value index
              if isAddress expectedState then ilg.Emit(OpCodes.Readonly)
            ...
            | Quotations.Patterns.FieldGet(objOpt, field) -> 
              ...
              | _ -> 
                ...
                | Some e -> 
                  let s = 
                    if e.Type.IsValueType then ExpectedStackState.Address
                    else ExpectedStackState.Value
                  emit s e
                let field = 
                  match field with
                  | :? ProvidedField as pf when fieldMap.ContainsKey pf -> 
                    fieldMap.[pf] :> FieldInfo
                  | m -> m
                if field.IsStatic then ilg.Emit(OpCodes.Ldsfld, field)
                else ilg.Emit(OpCodes.Ldfld, field)
            | Quotations.Patterns.FieldSet(objOpt, field, v) -> 
              ...
              let field = 
                ...
                | :? ProvidedField as pf when fieldMap.ContainsKey pf -> 
                  fieldMap.[pf] :> FieldInfo
                | m -> m
              if field.IsStatic then ilg.Emit(OpCodes.Stsfld, field)
              else ilg.Emit(OpCodes.Stfld, field)
            | Quotations.Patterns.Call(objOpt, meth, args) -> 
              ...
              let getMeth (m : MethodInfo) = 
                match m with
                | :? ProvidedMethod as pm when methMap.ContainsKey pm -> 
                  methMap.[pm] :> MethodInfo
                | m -> m
              ...
              let mappedMeth = 
                if meth.IsGenericMethod then 
                  ...
                  let gdty = 
                    convType (meth.DeclaringType.GetGenericTypeDefinition())
                  let gdtym = 
                    gdty.GetMethods() |> Seq.find (fun x -> x.Name = meth.Name)
                  assert (gdtym <> null)
                  let dtym = 
                    ...
                    | dty -> 
                      MethodBase.GetMethodFromHandle
                        (meth.MethodHandle, dty.TypeHandle) :?> _
                  assert (dtym <> null)
                  dtym
                else getMeth meth
              ...
              | Some obj when mappedMeth.IsAbstract || mappedMeth.IsVirtual -> 
                if obj.Type.IsValueType then 
                  ilg.Emit(OpCodes.Constrained, convType obj.Type)
            ...
            | Quotations.Patterns.NewObject(ctor, args) -> 
              ...
              let meth = 
                match ctor with
                | :? ProvidedConstructor as pc when ctorMap.ContainsKey pc -> 
                  ctorMap.[pc] :> ConstructorInfo
                | c -> c
            ...
            | Quotations.Patterns.Value(obj, _ty) -> 
              let rec emitC (v : obj) = 
                ...
                | :? decimal as x -> 
                  ...
                  do let sign = (bits.[3] &&& 0x80000000) <> 0
                     ilg.Emit(if sign then OpCodes.Ldc_I4_1
                              else OpCodes.Ldc_I4_0)
                  do let scale = byte ((bits.[3] >>> 16) &&& 0x7F)
            ...
            | Quotations.Patterns.TryWith(body, _filterVar, _filterBody, 
                                          catchVar, catchBody) -> 
              let stres, ldres = 
            ...
            | Quotations.Patterns.VarSet(v, e) -> 
              ...
              | false, _ -> 
                failwith 
                  "unknown parameter/field in assignment. Only assignments to locals are currently supported by TypeProviderEmit"
            | Quotations.Patterns.Lambda(v, body) -> 
              emitLambda 
                (ilg, v, body, expr.GetFreeVars(), locals, parameterVars)
              popIfEmptyExpected expectedState
            | n -> 
        ...
        for pcinfo in ctors do
          ...
          | Some f -> 
            ...
            for argExpr in argExprs do
              emitExpr (ilg, locals, parameterVars) ExpectedStackState.Value 
                argExpr
            ilg.Emit(OpCodes.Call, cinfo)
          if pcinfo.IsImplicitCtor then 
            for ctorArgsAsFieldIdx, ctorArgsAsField in List.mapi 
                                                         (fun i x -> (i, x)) 
                                                         implicitCtorArgsAsFields do
              ilg.Emit(OpCodes.Ldarg_0)
          ...
          ilg.Emit(OpCodes.Ret)
        match ptd.GetConstructors(ALL) |> Seq.tryPick (function 
                                            | :? ProvidedConstructor as pc when pc.IsTypeInitializer -> 
                                              Some pc
                                            | _ -> None) with
        | None -> ()
        | Some pc -> 
          ...
          emitExpr (ilg, new Dictionary<_, _>(), [||]) ExpectedStackState.Empty 
            expr
          ilg.Emit OpCodes.Ret
        for minfo in ptd.GetMethods(ALL) do
          match minfo with
          | :? ProvidedMethod as pminfo -> 
            ...
            let expectedState = 
              if (minfo.ReturnType = typeof<System.Void>) then 
                ExpectedStackState.Empty
              else ExpectedStackState.Value
        ...
        for evt in ptd.GetEvents(ALL) |> Seq.choose (function 
                                           | :? ProvidedEvent as pe -> Some pe
                                           | _ -> None) do
          let eb = 
            tb.DefineEvent(evt.Name, evt.Attributes, evt.EventHandlerType)
          defineCustomAttrs eb.SetCustomAttribute 
            (evt.GetCustomAttributesDataImpl())
          eb.SetAddOnMethod(methMap.[evt.GetAddMethod(true) :?> _])
          eb.SetRemoveOnMethod(methMap.[evt.GetRemoveMethod(true) :?> _])
        for pinfo in ptd.GetProperties(ALL) |> Seq.choose (function 
                                                 | :? ProvidedProperty as pe -> 
                                                   Some pe
                                                 | _ -> None) do
          let pb = 
            tb.DefineProperty
              (pinfo.Name, pinfo.Attributes, convType pinfo.PropertyType, 
               [| for p in pinfo.GetIndexParameters() -> 
                    convType p.ParameterType |])
...
type ProvidedAssembly(assemblyFileName : string) = 
  ...
  let assemblyLazy = 
    lazy assemblyGenerator.Generate(theTypes |> Seq.toList)
         assemblyGenerator.Assembly
  
  let add (providedTypeDefinitions : ProvidedTypeDefinition list, 
           enclosingTypeNames : string list option) = 
    for pt in providedTypeDefinitions do
      if pt.IsErased then 
        invalidOp 
          ("The provided type " + pt.Name 
           + "is marked as erased and cannot be converted to a generated type. Set 'IsErased' to false on the ProvidedTypeDefinition")
      theTypes.Add(pt, enclosingTypeNames)
      pt.SetAssemblyLazy assemblyLazy
  
  member x.AddNestedTypes(providedTypeDefinitions, enclosingTypeNames) = 
    add (providedTypeDefinitions, Some enclosingTypeNames)
  member x.AddTypes(providedTypeDefinitions) = 
    add (providedTypeDefinitions, None)

module Local = 
  let makeProvidedNamespace (namespaceName : string) 
      (types : ProvidedTypeDefinition list) = 
    let types = 
      [| for ty in types -> ty :> Type |]
...
type TypeProviderForNamespaces(namespacesAndTypes : list<string * list<ProvidedTypeDefinition>>) = 
  ...
  new(namespaceName : string, types : list<ProvidedTypeDefinition>) = 
    new TypeProviderForNamespaces([ (namespaceName, types) ])
  ...
  interface System.IDisposable with
    member x.Dispose() = disposing.Trigger(x, EventArgs.Empty)
  
  member __.AddNamespace(namespaceName, types : list<_>) = 
    otherNamespaces.Add(namespaceName, types)
  member __.Namespaces = Seq.readonly otherNamespaces
  member self.Invalidate() = invalidateE.Trigger(self, EventArgs())
  interface ITypeProvider with
    
    [<CLIEvent>]
    member this.Invalidate = invalidateE.Publish
    
    member this.GetNamespaces() = Array.copy providedNamespaces.Value
    ...
    member __.GetInvokerExpression(methodBase, parameters) = 
      let rec getInvokerExpression (methodBase : MethodBase) parameters = 
        ...
        | :? ProvidedMethod as m when (match methodBase.DeclaringType with
                                       | :? ProvidedTypeDefinition as pt -> 
                                         pt.IsErased
                                       | _ -> true) -> 
          m.GetInvokeCodeInternal false parameters |> expand
        | :? ProvidedConstructor as m when (match methodBase.DeclaringType with
                                            | :? ProvidedTypeDefinition as pt -> 
                                              pt.IsErased
                                            | _ -> true) -> 
          m.GetInvokeCodeInternal false parameters |> expand
        ...
        | :? System.Reflection.MethodInfo as minfo -> 
          if minfo.IsStatic then 
            ...
            Quotations.Expr.Call
              (parameters.[0], minfo, Array.toList parameters.[1..])
        | _ -> 
          failwith 
            ("TypeProviderForNamespaces.GetInvokerExpression: not a ProvidedMethod/ProvidedConstructor/ConstructorInfo/MethodInfo, name=" 
             + methodBase.Name + " class=" + methodBase.GetType().FullName)
      
      and expand expr = 
        match expr with
        | Quotations.Patterns.NewObject(ctor, args) -> 
          getInvokerExpression ctor [| for arg in args -> expand arg |]
        ...
        | Quotations.ExprShape.ShapeLambda(v, body) -> 
          Quotations.Expr.Lambda(v, expand body)
        | Quotations.ExprShape.ShapeCombination(shape, args) -> 
          Quotations.ExprShape.RebuildShapeCombination
            (shape, List.map expand args)
    ...
    member this.GetStaticParameters(ty) = 
      match ty with
      | :? ProvidedTypeDefinition as t -> 
        if ty.Name = t.Name then t.GetStaticParameters()
        else [||]
      | _ -> [||]
    
    member this.ApplyStaticArguments(ty, typePathAfterArguments : string [], 
                                     objs) = 
      let typePathAfterArguments = 
        typePathAfterArguments.[typePathAfterArguments.Length - 1]
      match ty with
      | :? ProvidedTypeDefinition as t -> 
        (t.MakeParametricType(typePathAfterArguments, objs) :> Type)
      | _ -> 
        failwith 
          (sprintf 
             "ApplyStaticArguments: static params for type %s are unexpected" 
             ty.FullName)
    
    member x.GetGeneratedAssemblyContents(_assembly) = 
      match System.Windows.Application.GetResourceStream
              (System.Uri("FSharp.Core.dll", System.UriKind.Relative)) with
      | null -> 
        failwith 
          "FSharp.Core.dll not found as Manifest Resource, we're just trying to read some random .NET assembly, ok?"
      | resStream -> 
