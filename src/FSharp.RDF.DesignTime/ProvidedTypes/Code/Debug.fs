...
module internal Debug = 
  ...
  let inline private separatedBy delimiter (items : string seq) = 
  ...
  let generate (resolutionFolder : string) (runtimeAssembly : string) 
      typeProviderForNamespacesConstructor args = 
    let cfg = new TypeProviderConfig(fun _ -> false)
    let (?<-) cfg prop value = 
      cfg.GetType().GetProperty(prop).GetSetMethod(nonPublic = true)
         .Invoke(cfg, [| box value |]) |> ignore
  ...
  let prettyPrint signatureOnly ignoreOutput maxDepth maxWidth 
      (t : ProvidedTypeDefinition) = 
    let ns = 
    ...
    let rec toString useFullName (t : Type) = 
      if t = null then "<NULL>" // happens in the Freebase provider
      else 
        ...
        let innerToString (t : Type) = 
          ...
          | t when t.IsGenericType -> 
            let args = 
              if useFullName then 
                t.GetGenericArguments() |> Seq.map (if hasUnitOfMeasure then 
                                                      (fun t -> t.Name)
                                                    else toString useFullName)
              else t.GetGenericArguments() |> Seq.map (fun _ -> "_")
            if FSharpType.IsTuple t then separatedBy " * " args
              ...
              let name, reverse = 
                match t with
                | t when hasUnitOfMeasure -> 
                  toString useFullName t.UnderlyingSystemType, false
                | t when t.GetGenericTypeDefinition().Name = typeof<int seq>
                           .GetGenericTypeDefinition().Name -> "seq", true
                | t when t.GetGenericTypeDefinition().Name = typeof<int list>
                           .GetGenericTypeDefinition().Name -> "list", true
                | t when t.GetGenericTypeDefinition().Name = typeof<int option>
                           .GetGenericTypeDefinition().Name -> "option", true
                | t when t.GetGenericTypeDefinition().Name = typeof<int ref>
    ...
    let printExpr expr = 
      ...
      let inline getAttrs attrName m = 
        ((^a : (member GetCustomAttributesData : unit
                   -> IList<CustomAttributeData>) m)) 
        |> Seq.filter 
    ...
    let printMember (memberInfo : MemberInfo) = 
      ...
      let getConstructorBody (c : ProvidedConstructor) = 
        if c.IsImplicitCtor then Expr.Value(())
        else 
          seq { 
      ...
      | :? ProvidedLiteralField as field -> 
        ...
        if not ignoreOutput then 
          print 
          <| "val " + field.Name + ": " + (toString true field.FieldType) 
             + value
      | :? ProvidedProperty as prop -> 
        if not ignoreOutput then 
          print 
          <| (if prop.IsStatic then "static "
              else "") + "member " + prop.Name + ": " 
